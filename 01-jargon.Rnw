...
\paragraph{Hypertext Documentation:} R is shipped with a comprehensive
...
packages. Only when a package is loaded (attached) its content is
available.
...
\textbf{repository} like CRAN (the default repository).
...
installed from CRAN into a local \textbf{library tree}.
...
\paragraph{Repository:} Repositories are places (websites) where a set
of packages are hosted and ready for installation. Prominent ones are
...
\paragraph{Library:} Libraries or library trees are local directories
where packages are installed (by \rcode{install.packages())} and
looked for (by \rcode{library()} and \rcode{require()}). Execute
...
\paragraph{Package content:} An R package typically contains some
functions, data sets, and documentation of both. To get an overview
execute \rcode{library(help = "package")} or look at its hypertext
documentation (execute \rcode{help.start()} and follow the links
``Packages'' and the package title).

<<eval=FALSE>>=
library(help = "archetypes")
@

To get a list of a package's data sets and to load a specific one use
the \rcode{data()} function.
<<eval=FALSE>>=
data(package = "archetypes")
data("body", package = "archetypes")
@

Demos are longer examples that demonstrate some of the functionality
...
Vignettes are (typically) detailed descriptions of parts of a
...
operates in a (infinite) \textbf{read-eval-print loop} (REPL): the
user enters an expression (read), the expression is evaluated (eval),
and the result is displayed (print)---and this is repeated until the
user enters \rcode{q()}.
...
expressions in a separate script file (with the ending \rcode{.R}).
...
appropriate \textbf{editor} for R scripts.
...
scripts (in my point of view) are RStudio \citep{RStudio} and
Emacs+ESS \citep{ESS}.
