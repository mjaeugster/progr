...
etc.; see the chapter about namespaces) a help page with detailed
information is available. To get the information for a specific
object, use the function \rcode{help()} or the shortcut \rcode{?}.
...
usage section and a description of all arguments. It also can contain
a details section, references and links to similar functions (the ``see
also'' section). 

Often a help page has an example section to demonstrate the function's
usage. The examples on a help topic can normally be run by
\rcode{example("topic")}.
...
\paragraph{Object:} case-sensitive
\paragraph{Function:}
\paragraph{Comment:}
<<>>=
