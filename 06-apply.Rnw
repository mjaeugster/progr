...
wich treats computation in terms of a program state and statements
...
following \citep{Wikipedia-HigherOrderFunction}:
\begin{itemize}
...
R provides other common higher-order functions; see
\rcode{?Map}.
