...
To understand a function it is ofen useful to execute its expressions
...
Whenever now the function is called, its execution is interrupted, a
...
Another important function is \rcode{traceback()}. If an error occured
with an unidentifiable error message, this functions shows the
