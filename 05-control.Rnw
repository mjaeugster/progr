...
\begin{description}
  ...
  \item [Return:] stop the program, preventing any further execution;
...
\rcode{statement1} is evaluted; if it is \rcode{FALSE} then \rcode
...
In case of the for loop, the number of interations is defined (i.e.,
